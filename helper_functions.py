...
def search_windows(img, windows, clf, scaler, color_space='RGB', 
    ...
    for window in windows:
        ...
        if img_seg.size > 0:
            test_img = cv2.resize(img_seg, (64, 64))      
...
def detect_cars(image, color_space, clf, X_scaler, orient, 
        pix_per_cell, cell_per_block, spatial_size, hist_bins):
    heat = np.zeros_like(image[:,:,0]).astype(np.float)
    draw_image = np.copy(image)

    # mpimg reads png files normalized 0-1, while jpeg files 0-255
    # training data was png, however test image is a jpg
    # normailize the image 
    image = image.astype(np.float32)/255
    y_start_stop = [400, image.shape[1]] # Min and max in y to search in slide_window()
...
def find_cars(img, hot_windows, color_space, xstart_stop, ystart, ystop, scale, svc, X_scaler, orient, 
    pix_per_cell, cell_per_block, spatial_size, hist_bins):
    
    img_tosearch = img[ystart : ystop, xstart_stop[0] : xstart_stop[1],:]
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            prob_pos = svc.decision_function(test_features)

            if test_prediction == 1 and prob_pos >= 0.1:

                xbox_left = np.int(xleft  * scale + xstart_stop[0])
